...
# Initialization
Gnet_ws = generator(ip_g, op_g, 1024, 1024, 512, 512)
Gnet_sw = generator(ip_g, op_g, 1024, 1024, 512, 512)
Dnet_w = discriminator(ip_d, op_d, 1024, 1024, 512, 512)
Dnet_s = discriminator(ip_d, op_d, 1024, 1024, 512, 512)
...
if isTrain:
    ...
    for ep in range(epoch):
        ...
        if (ep+1)%5==0:
            ...
            torch.save(Gnet_sw, join(mainfolder,"gen_sw_Ep_{}.pth".format(ep+1)))
    ...
    Gnet = torch.load(join(mainfolder,"gen_ws_Ep_100.pth"), map_location='cpu')
    for i in range(n):
